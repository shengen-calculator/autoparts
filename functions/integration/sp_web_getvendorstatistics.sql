...
BEGIN
	...
	WHERE        (dbo.[Запросы клиентов].Подтверждение = 0) AND (dbo.[Запросы клиентов].Интернет = 1) AND (dbo.[Запросы клиентов].Обработано = 0) AND (dbo.[Запросы клиентов].Заказано > 0) AND
							 (dbo.[Запросы клиентов].ID_Клиента <> 378) AND (dbo.[Запросы клиентов].Дата > CONVERT(DATETIME, @startDate, 102)) AND (dbo.[Запросы клиентов].Дата < CONVERT(DATETIME, @endDate, 102))
