CREATE PROCEDURE sp_web_addorder @clientId int,
                                 @productId int,
                                 @price decimal(9, 2),
                                 @isEuroClient bit,
                                 @quantity int,
...
BEGIN
    ...
    SELECT @brandName = dbo.[Каталоги поставщиков].Брэнд,
           ...
           @description = dbo.[Каталоги поставщиков].Описание,
    ...
    WHERE (ID_Брэнда LIKE @brandId)
      AND ([namepost] LIKE @shortNumber)
      AND (ID_Поставщика LIKE @vendorId)
    ...
    IF (@@ROWCOUNT = 0)
        BEGIN
            ...
            IF (@@ROWCOUNT = 0)
                BEGIN
                    INSERT INTO [Каталог запчастей] (ID_Поставщика, ID_брэнда,
                                                     [номер запчасти], [Номер поставщика], ID_Аналога, Описание, Цена,
                                                     Исполнитель)
            ...
            ELSE
                BEGIN
                    INSERT INTO [Каталог запчастей] (ID_Поставщика, ID_брэнда,
                                                     [номер запчасти], [Номер поставщика], ID_Автомобиля,
                                                     [ID_Группы товаров],
