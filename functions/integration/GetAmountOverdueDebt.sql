CREATE FUNCTION [dbo].[GetAmountOverdueDebt](@clientId int,
                                             @dayQuantity int)
    RETURNS DECIMAL(9, 2)
AS
BEGIN
    ...
    SET @lastDay = DATEADD(day, @dayQuantity + 1, CAST(CURRENT_TIMESTAMP AS DATE))
    ...
    FROM (SELECT ID_Клиента, Количество, Цена, Грн, Количество * Цена AS Eur, Количество * Грн AS Uah
          ...
          WHERE ((Нету = 0) AND (Обработано = 1) AND (ID_Клиента = @clientId))
            AND ((Дата_закрытия < @lastDay) OR (Количество < 0))) AS derivedtbl_1
